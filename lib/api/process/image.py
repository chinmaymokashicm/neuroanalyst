...
from lib.process.create import ProcessImage
...
async def load_process_image_schema():
    schema = ProcessImage.get_ui_form_schema()
    return schema

@router.get("/create/", tags=["process", "image"])
...
async def preview_process(form_data: str = Form(...)):
    try:
...
async def create_process(form_data: str = Form(...)):
    try:
        form_dict: dict = json.loads(form_data)
