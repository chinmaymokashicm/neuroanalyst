...
- Provide programming language (current recommendation is only Python), logic (code in that logic), required input arguments, virtual environment (path if exists, plus installation script if it does not exist), and other metadata (e.g. name and description).
