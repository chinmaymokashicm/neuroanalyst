...
class PythonEncoder(BaseEncoder):
    ...
    def _format_code_body(self, code: str) -> str:
        ...
        # Calculate minimum indentation in the original code to preserve relative indentation
        non_empty_lines = [line for line in lines if line.strip()]
        if non_empty_lines:
            # Find leading spaces in each non-empty line
            leading_spaces = [len(line) - len(line.lstrip()) for line in non_empty_lines]
            # Get the minimum (base) indentation if any lines have indentation
            min_indentation = min(leading_spaces) if leading_spaces and min(leading_spaces) > 0 else 0
        else:
            min_indentation = 0
        ...
        for line in lines:
            if line.strip():  # Non-empty line
                ...
                # Preserve relative indentation by removing base indentation and adding standard indentation
                if current_indentation >= min_indentation:
                    relative_indent = current_indentation - min_indentation
                    formatted_lines.append(' ' * self.config.indent_size + ' ' * relative_indent + line.lstrip())
                else:
                    # Fallback for lines with less than minimum indentation
