...
from .executor import ProcessStatus, BaseExecutor, LSFExecutor, SLURMExecutor, PBSExecutor, LocalExecutor
...
class NeuPipeline(BaseModel):
    ...
    def find_exec_by_id(self, exec_id: str) -> Optional[NeuProcessExec]:
        """Find a NeuProcessExec instance in the pipeline by its exec_id."""
    ...
    def execute_via_python(self, resume: bool = True, window_size: int = 8) -> str:
        ...
        Args:
            resume: If True, resume execution from the last successful step.
            window_size: The maximum number of concurrent processes to run per step.
        ...
        for step_index in range(starting_step_index, len(self.steps)):
            ...
            executors: dict[str, LSFExecutor | SLURMExecutor | PBSExecutor | LocalExecutor] = {}
            status_lock = Lock()  # For thread-safe status updates
            failure = False
            failure_message = ""
            
            # Start all processes in this step concurrently - only those not yet complete
            step_status = self.get_pipeline_status().steps[step_index]
            status_by_exec = {p.exec_id: p.status for p in step_status.processes}

            incomplete_execs = [
                pe for pe in step.process_execs
                if status_by_exec.get(pe.exec_id) != ProcessStatus.COMPLETE.value
            ]
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=window_size) as thread_pool:
                while (pending_execs or running_futures) and not failure:
                    # Fill window
                    while len(running_futures) < window_size and pending_execs:
                        proc_exec = pending_execs.popleft()
                        future = thread_pool.submit(execute_and_monitor_process, proc_exec)
                        running_futures[future] = proc_exec.exec_id
                        logger.info(
                            f"Started process {proc_exec.exec_id}. "
                            f"Running: {len(running_futures)}/{window_size}"
                        )
                    ...
                    for future in done:
                        exec_id = running_futures.pop(future)
                        try:
                            ...
                            logger.error(f"Exception in process {exec_id}: {e}")
                            failure = True
                            break
                ...
                if failure:
                    for exec_id, exec_obj in executors.items():
                        if not exec_obj.is_done():
                            logger.warning(
                                f"Process {exec_id} still running; "
                                f"scheduler-level cancellation not implemented"
                            )
                    return failure_message or f"Pipeline {self.pipeline_id} halted due to failure."
    ...
    def kill_job(self, exec_id: str) -> bool:
        """Kill a running job associated with the given process execution ID.
        
        Args:
            exec_id: Process execution ID whose job needs to be killed.
            
        Returns:
            bool: True if the job was successfully killed, False otherwise.
        """
        # Identify the Job ID of the exec_id.
        def find_job_id(exec_id: str) -> Optional[str]:
            pipeline_status: NeuPipelineStatus = self.get_pipeline_status()
            for step_status in pipeline_status:
                for exec_status in step_status:
                    if exec_id == exec_status.exec_id:
                        return exec_status.scheduler_job_id
        
        # Generate kill command based on scheduler
        if self.scheduler == HPCScheduler.LOCAL:
            command: str = f"kill {find_job_id(exec_id)}"
        elif self.scheduler == HPCScheduler.LSF:
            command: str = f"bkill {find_job_id(exec_id)}"
        elif self.scheduler == HPCScheduler.PBS:
            command: str = f"qdel {find_job_id(exec_id)}"
        elif self.scheduler == HPCScheduler.SLURM:
            command: str = f"scancel {find_job_id(exec_id)}"
        ...
        try:
            ...
            self.logger.info(f"Successfully killed job for exec_id {exec_id}. Output:\n{result.stdout}")
            return True
        except subprocess.CalledProcessError as e:
            self.logger.error(f"Failed to kill job for exec_id {exec_id}. Error:\n{e.stderr}")
            return False
        
    def get_all_jobs(self) -> list[dict]:
        """Get a list of all jobs associated with this pipeline.
        
        Returns:
            list[dict]: A list of job details including exec_id, job_id, status.
        """
        jobs = []
        pipeline_status: NeuPipelineStatus = self.get_pipeline_status()
        for step_status in pipeline_status.steps:
            for exec_status in step_status.processes:
                if exec_status.scheduler_job_id:
                    jobs.append({
                        "exec_id": exec_status.exec_id,
                        "job_id": exec_status.scheduler_job_id,
                        "status": exec_status.status
                    })
        return jobs
    
    def kill_all_jobs(self) -> dict:
        """Kill all running jobs associated with this pipeline.
        
        Returns:
            dict: A dictionary with exec_id as keys and boolean indicating success of kill operation.
        """
        results = {}
        jobs = self.get_all_jobs()
        for job in jobs:
            success = self.kill_job(job["exec_id"])
            results[job["exec_id"]] = success
        return results