...
def fsl_threshold(input_filepath: str):
    ...
    csf_mask = nib.Nifti1Image(np.zeros((10, 10, 10)), affine=np.eye(4)).get_fdata()  # Placeholder data for testing
    gm_mask = nib.Nifti1Image(np.zeros((10, 10, 10)), affine=np.eye(4)).get_fdata()  # Placeholder data for testing
    wm_mask = nib.Nifti1Image(np.zeros((10, 10, 10)), affine=np.eye(4)).get_fdata()  # Placeholder data for testing
    output_data = np.stack([csf_mask, gm_mask, wm_mask], axis=-1)
