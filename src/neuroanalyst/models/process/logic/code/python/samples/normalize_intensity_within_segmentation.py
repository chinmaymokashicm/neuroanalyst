...
def normalize_intensity_within_segmentation(input_filepath: str):
    ...
    Returns:
        output_data (np.ndarray): Intensity-normalized dual-channel image data. Shape : (X, Y, Z, 2).
            ...
            2nd channel: segmentation labelmap
    ...
    image = input_data[..., 0]
    labelmap = input_data[..., 1]
    
    normalized_image = np.copy(image)
    unique_labels = np.unique(labelmap.astype(np.int32))
    for label in unique_labels:
        if label == 0:
            continue  # Skip background
        region_voxels = image[labelmap == label]
        mean_intensity = np.mean(region_voxels)
        std_intensity = np.std(region_voxels)
        if std_intensity > 0:
            normalized_image[labelmap == label] = (region_voxels - mean_intensity) / std_intensity
        else:
            normalized_image[labelmap == label] = 0.0  # If std is zero, set to zero
    
    output_data = np.stack([normalized_image, labelmap], axis=-1)
    ...
    metrics = {
        ...
        "normalization": "zscore_within_label" # Normalization method used - can be used by downstream processes to check if normalization was applied
    }
